function Scatterplot() {
        ...
        var brush = d3.svg.brush()
            ...
            .on("brush", function () {
                ...
                d3.selectAll(attr.nodeSelector + '.node')// NODES ONLY, CENTROIDS DON'T MATTER
                    .attr("fill", function (d) {
                        ...
                        if (selected) brushSelected[d.content.id] = d.content;
            ...
            .on("brushend", function() {
                ...
                if (!selectedKeys.length){
                    svg.selectAll(attr.nodeSelector + '.node').attr("fill", function (d) { return d.fillbackup; });
                }
                else {
                    var data = attr.data.filter(function (d) { // if NOT centroid and IN selected keys
                        if (d.content) return Utils.inArray(d.content.id.toString(), selectedKeys)
                    });
                    ...
                    new ScatterplotBrushed().init(
                        {nodes: data}, selectedKeys, 350, 140, '#selections-area ul', brushID, attr.d3Color, borderColor);
    ...
    return {
        ...
        init: function(root, width, height, selector, id) {
            ...
            attr.term_freq = root.term_frequencies;
            attr.tfidf = root.tfidf;
