...
def read_save_to_db(attrs, file_path, timestamp):
    ...
    if timestamp != get_last_timestamp():
        ...
        log_info('New file sent. Saving to db...'.format(timestamp))
        ...
        log_info('No new timestamp sent, using the most recent {}...'.format(get_last_timestamp()))


def read_save_csv(file_path, timestamp, delimiter):
    """Read csv file using pandas, and save it to the database."""
    data = pd.read_csv(file_path, sep=delimiter, error_bad_lines=False)
