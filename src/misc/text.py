...
stop = lambda attrs: stopwords if attrs['stopwords'] == 'true' else None
norm = lambda attrs: attrs['norm'] if attrs['norm'] != 'none' else None
join_by_cluster = lambda cl, df, tc: ' '.join(df[df.cluster == cl][tc].apply(' '.join, axis=1).values)
...
def tfidf(attrs, id_list=None):
    ...
    stop_ = stop(attrs)
    norm_ = norm(attrs)
    features = attrs['featureNumber']

    # If an id list is given, get results for these items only
    if id_list:
        pass

    # Consider each cluster a single document,
    # so get the text from each cluster separately
    text_by_cluster = [join_by_cluster(cluster, df, text_columns)
                       for cluster in df.cluster.unique()]

    vec = TfidfVectorizer(max_features=features, norm=norm_, token_pattern=TOKEN_PATTERN, stop_words=stop_)
    tfidf_matrix = vec.fit_transform(text_by_cluster).toarray()
    feature_names = vec.get_feature_names()

    # Get the top 10 TF-IDF scores for each cluster
    tfidf_by_cluster = {}
    for cluster, doc in enumerate(tfidf_matrix):
